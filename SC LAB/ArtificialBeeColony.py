...
class ArtificialBeeColony:
    ...
    def __init__(self, D, Lb, Ub, n, generation, ans, func):
        ...
        self.probabilityArray = np.zeros(n)
    ...
    def doRun(self):
        ...
        for gen in range(self.generation):
            ...
            #Onlooker Bee Phase
            for i in range(self.n):
                self.probabilityArray[i] = self.fitnessArray[i]/self.fitnessArray.sum()
            for i in range(self.n):
                if(rand.random() < self.probabilityArray[i]):
                    self.updateSolution(i)
...
abc = ArtificialBeeColony(2, -5, 5, 5, 10, 0, test)
abc.doRun()